...
@lightbulb.option("category", "Which category of commands are you interested in?", required=False, choices=["All Commands", "Fun Commands", "Utility Commands", "Privacy", "Confessions"])
...
async def help_command(ctx: lightbulb.SlashContext):
    ...
    try:
        ...
        if category == None:
            ...
            embed = hikari.Embed(title="Privacy Information", description=message)
            embed.set_thumbnail(thumbnail)
            ...
            embed = hikari.Embed(title="Confessions", description=message)
            embed.set_thumbnail(thumbnail)
