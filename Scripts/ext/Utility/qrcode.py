...
@lightbulb.option("data", "The data that the QR code should contain", type=str, required=True)
@lightbulb.option("scale", "The scale of each pixel in pixels", type=int, min_value=1, max_value=100, required=False, default=8)
...
@lightbulb.option("image", "A background image/gif (doesn't allow for colors)", type=hikari.OptionType.ATTACHMENT, required=False, default=None)
@lightbulb.option("icon", "Put an icon in the middle (doesn't work with image)", type=hikari.OptionType.ATTACHMENT, required=False, default=None)
@lightbulb.command("qrcode", "Generate QR Code", pass_options=True, auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashCommand)
async def qrcode_command(ctx: lightbulb.SlashContext,
                         data: str, scale: int,
    ...
    if image:
        ...
        if image_extension not in allowed_files:
            await ctx.respond(f"The image file must be one of the following types: {', '.join(allowed_files)}.")
            return
        ...
        # Validate colors
        if not await QRCode.validate_colors(color_background, color_data, color_outline):
            await ctx.respond("One or more colors are invalid. Please provide valid CSS color codes, hex values, or RGB values.")
            return

    icon_img = None
    if icon:
        if os.path.splitext(icon.filename)[1].lower() not in [".png", ".jpg", ".jpeg"]:
            await ctx.respond("Your icon file is not supported. Please upload either .png, .jpg or .jpeg")
            return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(icon.url) as resp:
                if resp.status == 200:
                    ...
                    await ctx.respond(f"Couldn't download your icon. Error code: {resp.status}")
                    return

    try:
        qrcode = await QRCode.generate_qrcode(data=data, filename=id, scale=scale, color_background=color_background, color_data=color_data, color_outline=color_outline, image=image, icon=icon_img)
    except segno.encoder.DataOverflowError:
        await ctx.respond("Data too large. No (Micro) QR Code can handle the provided data")
        return
    ...
    embed = hikari.Embed(title="Here's your QR Code", description=f"Here's your generated QR code with the data:\n```{data}```")
