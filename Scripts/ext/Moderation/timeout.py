...
async def timeout_command(ctx: lightbulb.SlashContext, user: hikari.Member, seconds: int, minutes: int, hours: int, days: int, reason: str):
    """
    ...
    try:
        ...
        await ctx.respond("I do not have permission to timeout the user.", flags=hikari.MessageFlag.EPHEMERAL)
        return
    ...
    try:
        if days == 0 and hours == 0 and minutes == 0 and seconds == 0:
            ...
            await user.send(f"You have been timed out until <t:{int(then.timestamp())}:R> for `{reason}`")
    except Exception as e:
        from bot import logger
