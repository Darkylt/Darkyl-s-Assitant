...
async def warn_member(user_id: int, reason: str):
    ...
    if 2 < warnings_count < 4:
        from bot import kick_user
        await kick_user(user_id, "Too many warnings")
    elif warnings_count >= 4:
        from bot import ban_user
        await ban_user(user_id, "Too many warnings")
