...
async def announce_last_video(latest_video, bot):
    ...
    msg = await bot.application.app.rest.create_message(
        config.YouTube.discord_channel, message, role_mentions=True
    )
    await bot.application.app.rest.crosspost_message(config.YouTube.channel, msg)
