...
async def kick_user(user_id: int, reason: str, guild=config.Bot.server):
    try:
        await bot.application.app.rest.kick_user(guild=guild, user=user_id, reason=reason)
        return True
    except hikari.errors.ForbiddenError:
        return False

async def ban_user(user_id: int, reason: str, delete_message_seconds: int = 0,guild=config.Bot.server):
    try:
        await bot.application.app.rest.ban_user(guild=guild, user=user_id, reason=reason, delete_message_seconds=delete_message_seconds)
        return True
    except hikari.errors.ForbiddenError:
        return False
