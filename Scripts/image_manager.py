...
async def download_image(image_url, id):
    ...
    image_path = os.path.join(config.Paths.data_folder, "Downloaded Profile Pictures", f"{id}.{file_extention}")
...
async def make_love_images(user1_path, user2_path, user1_id, user2_id) -> str:
    ...
    overlayed_image_path = str(os.path.join(config.Paths.data_folder, "Downloaded Profile Pictures", f"{user1_id, user2_id}.png"))
...
async def quote_generator(message: str, username: str, pfp: str, date: str) -> str:
    ...
    background_image = await utils.choose_random_file(os.path.join(config.Paths.assets_folder, "Quote Images"))
    ...
    id = await utils.generate_id(os.path.join(config.Paths.data_folder, "Generated Quotes"))
    ...
    quote_image_path = os.path.join(config.Paths.data_folder, "Generated Quotes", f"{username}_{id}.png")
