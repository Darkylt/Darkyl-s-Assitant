    ...
    for guild_id, bans in ban_data.items():
        for user_id, ban_info in bans.items():
            ...
            if unban_time > current_time:
                ...
                try:
                    await plugin.bot.application.app.rest.unban_user(guild=int(guild_id), user=int(user_id))
...
@lightbulb.option("duration", "How long should the ban last? (e.g., 1y2d3h29m2s)", required=True, type=str)
@lightbulb.command("tempban", "Ban a user temporarily", pass_options=True)
@lightbulb.implements(lightbulb.SlashCommand)
async def tempban_command(ctx: lightbulb.SlashContext, user: hikari.Member, reason: str, delete_messages, duration: str):
    """
    ...
    try:
        ...
        if ban_duration is None:
            await ctx.respond("Invalid duration format. Please use a format like '1y2d3h29m2s'.", flags=hikari.MessageFlag.EPHEMERAL)
            return
        ...
        ban_data[str(ctx.guild_id)][str(user.id)] = {
            "unban_time": (datetime.utcnow() + timedelta(seconds=ban_duration)).timestamp()
        }
