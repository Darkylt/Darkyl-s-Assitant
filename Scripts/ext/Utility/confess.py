...
@lightbulb.option("image", "An image to attach", hikari.OptionType.ATTACHMENT, required=False, default=None)
...
async def confess_command(ctx: lightbulb.SlashContext, text: str, image: hikari.File):
    ...
    if image:
        ...
        if not filetype in supported_formats:
            ...
            await ctx.respond(f"Discord only supports the following filetypes:{txt}", flags=hikari.MessageFlag.EPHEMERAL)
            return

        image_path = await image_manager.download_image(image.url, id, os.path.join(config.Paths.data_folder, "Confession Images"))

        if image_path == None:
            await ctx.respond("There was an error while attempting to download your image. Maybe it's in the wrong file format?", flags=hikari.MessageFlag.EPHEMERAL)
            return
    ...
    embed = hikari.Embed(title=f"Confession {id}", description=text, color=hikari.Color.from_hex_code("#36ff6b"))
    ...
    message = await plugin.app.rest.create_message(channel=config.Channels.confession_approving_channel, embed=embed, components=view)
    ...
    await ctx.respond(f"Your confession has been submitted with the ID *{id}*. A moderator will either approve or deny the confession. This may take a little time.", flags=hikari.MessageFlag.EPHEMERAL)
