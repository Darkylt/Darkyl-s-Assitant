...
@lightbulb.command("mute", "Mute a user.", auto_defer = True, pass_options = True, ephemeral = True)
...
@lightbulb.app_command_permissions(hikari.Permissions.MODERATE_MEMBERS, dm_enabled=False)
@lightbulb.command("unmute", "Unmute a user.", auto_defer = True, pass_options = True, ephemeral = True)
@lightbulb.implements(lightbulb.SlashCommand)
...
@lightbulb.add_cooldown(3, 3, lightbulb.UserBucket)
@lightbulb.app_command_permissions(hikari.Permissions.MODERATE_MEMBERS, dm_enabled=False)
@lightbulb.command("configure_mute_role", "Give the mute role the proper restrictions.", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashCommand)
