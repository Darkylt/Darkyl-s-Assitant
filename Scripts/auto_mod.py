...
async def check_for_fakenews(content: str) -> bool:
    ...
    """
    try:
        banned_links_path = os.path.join(config.Paths.data_folder, "Banned Content", "Websites", "fakenews.txt")
...
async def check_for_gambling(content: str) -> bool:
    ...
    """
    try:
        banned_links_path = os.path.join(config.Paths.data_folder, "Banned Content", "Websites", "gambling.txt")
        with open(banned_links_path, 'r') as file:
            banned_links = file.read().splitlines()
...
async def check_for_porn(content: str) -> bool:
    ...
    try:
        ...
        banned_links_path = os.path.join(config.Paths.data_folder, "Banned Content", "Websites", "porn.txt")
        with open(banned_links_path, 'r') as file:
            banned_links = file.read().splitlines()
...
async def check_for_nswf(content: str) -> bool:
    ...
    try:
        banned_words_folder = os.path.join(config.Paths.data_folder, "Banned Content", "Words", "NSFW Words")
...
def update_report_data(user_id, message_id: int, violations: list):
    ...
    try:
        database_path = os.path.join(config.Paths.data_folder, "User Data", "reports.json")
        report_data = {}
