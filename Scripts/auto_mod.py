...
async def check_for_fakenews(content: str) -> bool:
    ...
    """
    try:
        banned_links_path = os.path.join(
            config.Paths.data_folder, "Banned Content", "Websites", "fakenews.txt"
        )
...
async def check_for_gambling(content: str) -> bool:
    ...
    try:
        banned_links_path = os.path.join(
            config.Paths.data_folder, "Banned Content", "Websites", "gambling.txt"
        )
        with open(banned_links_path, "r") as file:
            banned_links = file.read().splitlines()
...
async def check_for_porn(content: str) -> bool:
    ...
    try:
        ...
        banned_links_path = os.path.join(
            config.Paths.data_folder, "Banned Content", "Websites", "porn.txt"
        )
        with open(banned_links_path, "r") as file:
            banned_links = file.read().splitlines()
...
def create_report_embed(
    event: hikari.MessageCreateEvent, violations: list[str]
) -> hikari.Embed:
...
async def handle_violations(event: hikari.MessageCreateEvent, violations: list[str]):
