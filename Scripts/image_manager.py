...
async def download_image(image_url, id):
    ...
    if clean_image_url.endswith(".png"):
        ...
        logger.error("Couldn't define file extention of the avatar")
        return None

    image_path = os.path.join(config.Bot.data_folder, "Downloaded Profile Pictures", f"{id}.{file_extention}")
...
async def make_card(background_path, scale_factor=config.Join.welcome_card_scale_factor):
    # Open the images
    ...
    position = ((overlay_width - new_bg_width) // 2, (overlay_height - new_bg_height) // 2)
...
async def delete(image: str) -> bool:
    ...
    try:
        ...
        from bot import logger
        logger.error(f"Following error occurred while attempting to delete an image: {e}")
        return False
...
async def make_love_images(user1_path, user2_path, user1_id, user2_id) -> str:
    ...
    overlayed_image_path = str(os.path.join(config.Bot.data_folder, "Downloaded Profile Pictures", f"{user1_id, user2_id}.png"))
    user2_img.save(overlayed_image_path)
...
async def quote_generator(message: str, username: str, pfp: str, date: str) -> str:
    ...
    background_image = await utils.choose_random_file(os.path.join(config.Bot.assets_folder, "Quote Images"))

    image = Image.open(background_image)

    draw = ImageDraw.Draw(image)

    font_path = os.path.join(config.Bot.assets_folder, "Fonts", "Heavitas.ttf")
    font_size = 200
    ...
    if profile_picture_enabled:
        ...
        pfp_image = Image.composite(pfp_image, Image.new("RGBA", pfp_size, (255, 255, 255, 0)), mask)
    ...
    words = message.split()
    words_per_lines = 5
    formatted_message = '\n'.join([' '.join(words[i:i+5]) for i in range(0, len(words), words_per_lines)])
    ...
    id = await utils.generate_id(os.path.join(config.Bot.data_folder, "Generated Quotes"))
    ...
    quote_image_path = os.path.join(config.Bot.data_folder, "Generated Quotes", f"{username}_{id}.png")
