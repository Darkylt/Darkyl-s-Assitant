...
async def socials_command(ctx: lightbulb.SlashContext):
    ...
    if not await utils.validate_command(ctx):
        return
    try:
        thumbnail = hikari.File(os.path.join(config.Paths.assets_folder, "logo_300x300.png"))
        ...
        embed=hikari.Embed(title="**Darkyl's Social Media Profiles:**", description=f"Hi, I am Darkyl. Your gateway to a spectrum of {choice(random_text)} vibes straight outta Germany!", color=0x9900ff)
