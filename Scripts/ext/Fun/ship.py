...
async def ship_command(ctx: lightbulb.SlashContext):
    ...
    if await utils.nsfw_blacklisted(user1.id):
        await ctx.respond(f"{user1.mention} opted out from being included in this command", flags=hikari.MessageFlag.EPHEMERAL)
        return
    
    if await utils.nsfw_blacklisted(user2.id):
        await ctx.respond(f"{user2.mention} opted out from being included in this command", flags=hikari.MessageFlag.EPHEMERAL)
        return
    
    if await utils.nsfw_blacklisted(ctx.author.id):
        await ctx.respond("You have opted out of these commands.", flags=hikari.MessageFlag.EPHEMERAL)
        return
