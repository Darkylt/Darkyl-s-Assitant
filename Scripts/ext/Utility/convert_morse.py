...
@lightbulb.option("mode", "Convert Morse to ASCII or vice versa", str, required=True, choices=["text to morse", "morse to text"])
...
async def converter_binary_command(ctx: lightbulb.SlashContext, mode: str, value: str):
    ...
    if mode == "text to morse":
        if not is_valid_text(value):
            await ctx.respond("The text contains unsupported characters. Please use only letters, numbers, and basic punctuation.", flags=hikari.MessageFlag.EPHEMERAL)
            return
        result = encode(value)
    elif mode == "morse to text":
        if not is_valid_morse(value):
            await ctx.respond("The Morse code contains unsupported characters. Please use only '.', '-', '/', and spaces.", flags=hikari.MessageFlag.EPHEMERAL)
            return
        result = decode(value)
    ...
    if len(result) <= 480:
        ...
        try:
            ...
            logger.error(f"An error occurred in /converter_morse during sending to author: {e}")
