...
async def mod_menu_command(ctx: lightbulb.SlashContext, user: hikari.User):
    ...
    stats_file = os.path.join(config.Bot.data_folder, "User Data", "user_stats.json")
    with open(stats_file, "r") as file:
        stats_data = json.load(file)

    embed.add_field("**• Messages Sent:**", value=stats_data[str(user.id)]["message count"])
    embed.add_field("**• Commands Used:**", value=stats_data[str(user.id)]["commands used"])
    embed.add_field("**• XP:**", value=stats_data[str(user.id)]["xp"])
    embed.add_field("**• Level:**", value=stats_data[str(user.id)]["level"])
    ...
    embed.add_field("**• Reported user:**", value=stats_data[str(user.id)]["reported"])
