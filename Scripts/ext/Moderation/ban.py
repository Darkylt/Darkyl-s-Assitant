...
@lightbulb.option("delete_messages", "How far back should messages be deleted?", required=False, choices=list(time_durations.keys()), default=None)
@lightbulb.option("notify_user", "Should I notify the user about the ban?", bool, required=False, default=True)
@lightbulb.command("ban", "Ban a user", pass_options=True, auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashCommand)
async def ban_command(ctx: lightbulb.SlashContext, user: hikari.Member, reason: str, delete_messages: str, notify_user: bool) -> None:
    """
    ...
    try:
        ...
        if notify_user:
            try:
                ...
                if dm_channel:
                    await dm_channel.send(f"You have been banned from **{ctx.get_guild().name}** for: {reason}")
        ...
        await ctx.respond("Invalid option provided for message deletion time.", flags=hikari.MessageFlag.EPHEMERAL)
