...
def get_segment_statistic(segment: int, total_digits: int):
    ...
    for digit, count in sorted_frequency.items():
        ...
        if count == 1:
            formatted_string += f"**{digit}**: *{count} time,* {percentage:.4f}% of segment\n"
        else:
            formatted_string += f"**{digit}**: *{count} times,* {percentage:.4f}% of segment\n"
    ...
    formatted_string += f"Odd digits: {odd_count} times, {odd_percentage:.2f}% of segment"
    ...
    if total_gap_count > 0:
        ...
        formatted_string += f"Overall average gap between the same number: {overall_avg_gap:.2f}\n"
    else:
...
def generate_pie_chart(number, filename='pie_chart.png'):
    ...
    fig, ax = plt.subplots(figsize=(8, 8), facecolor='#171717')  # Set figure background color
...
@plugin.command
@lightbulb.option("start", "Where should the segment start", type=int, required=True, min_value=1)
...
async def pi_segment_command(ctx: lightbulb.context.SlashContext, start, length) -> None:
    ...
    try:
        ...
        stats_image_path = os.path.join(config.Paths.data_folder, "Generated Statistics", f"{await utils.generate_id()}.png")

        generate_pie_chart(segment, stats_image_path)

        embed = hikari.Embed(title="Here are some statistics about your segment:", description=stats)

        embed.set_author(name="Darkyl's Assistants PI segment command", icon=hikari.File(os.path.join(config.Paths.assets_folder, "pfp.png")))

        embed.set_thumbnail(hikari.File(os.path.join(config.Paths.assets_folder, "pi.png")))

        embed.set_image(hikari.File(stats_image_path))

        await ctx.respond(f"Your segment of pi at {start} is:\n```{segment}```", embed=embed)
