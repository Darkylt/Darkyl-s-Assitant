...
async def register_captcha(id: str, user_id: int, captcha_type: int, text: str, message_id: int):
    """
    ...
    Args:
        ...
        captcha_trpe (int): The type of captcha:
            1: Image Captcha
    """
    captchas = await read_db() or {}

    captchas[id] = [user_id, captcha_type, text, message_id]
...
async def delete_entries_from_user_id(user_id: int):
    ...
    try:
        ...
        updated_data = {captcha_id: details for captcha_id, details in data.items() if details[0] != user_id}
