...
class VerifyView(miru.View):
    ...
    async def basic_button(self, ctx: miru.ViewContext, button: miru.Button) -> None:
        ...
        if captcha_enabled:
            await ctx.respond("Extra verification required. The bot will DM you with a captcha", flags=hikari.MessageFlag.EPHEMERAL)
            from Verification.handle_captcha import handle_captcha
            await handle_captcha(ctx, button)
            return
...
class Report(miru.View):
    ...
    async def ignore_button(self, ctx: miru.ViewContext, button: miru.Button) -> None:
        ...
        database_path = os.path.join(config.Paths.data_folder, "Database", "reports.json")
...
class Confess(miru.View):
    ...
    @miru.button(label="Dismiss", style=hikari.ButtonStyle.DANGER, custom_id="confess_dismiss")
    ...
    @miru.button(label="Accpet", style=hikari.ButtonStyle.SUCCESS, custom_id="confess_accept")
    async def accept_button(self, ctx: miru.ViewContext, button: miru.Button) -> None:
        ...
        for key, value in confessions.items():
            ...
            await ctx.respond("Confession not found in database.", flags=hikari.MessageFlag.EPHEMERAL)
            return

        embed = hikari.Embed(title=f"Confession {confession_id}", description=text, color=hikari.Color.from_hex_code("#36ff6b"))
        ...
        message = await ctx.client.app.rest.create_message(config.Channels.confession_channel, embed=embed)
...
class Test(miru.View):
    ...
    @miru.button(label="Test Button", style=hikari.ButtonStyle.SUCCESS, custom_id="test_button", row=0)
