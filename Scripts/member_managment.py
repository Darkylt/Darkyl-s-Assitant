...
async def warn_member(user_id: int, reason: str) -> bool:
    ...
    """
    try:
        database_path = os.path.join(config.Paths.data_folder, "User Data", "warnings.json")
        data = {}
        ...
        if (config.AutoMod.kick_threshold-2) < warnings_count < (config.AutoMod.kick_threshold+1):
            try:
                await hikari.Application.app.rest.kick_user(guild=config.Bot.server, user=int(user_id), reason="Too many warnings")
            except hikari.errors.ForbiddenError:
                return False
        elif warnings_count >= (config.AutoMod.ban_threshold-1):
            try:
                await hikari.Application.app.rest.ban_user(guild=config.Bot.server, user=int(user_id), delete_message_seconds=0, reason="Too many warnings from moderators.")
            except hikari.errors.ForbiddenError:
                return False
...
async def update_user_stats(user_id: int, msg: bool, cmd: bool, rep: bool):
    """
    ...
    try:
        if database_interaction.update_user_entry(
            user_id=int(user_id), 
            increment=True, 
            msg_count=int(msg), 
            cmds_used=int(cmd), 
            ...
            try:
                await update_xp(user_id=int(user_id), report=rep)
            except Exception as e:
                from bot import logger
...
async def update_xp(user_id: int, report: bool) -> bool:
    ...
    user_data = database_interaction.get_user_entry(user_id=user_id)

    if user_data:
        id, msg_count, current_xp, last_level, cmds_used, reported, been_reported, nsfw_opt_out = user_data
    else:
        from bot import logger
        logger.error("During update_xp: User entry not found or error occurred.")

    if report:
        new_xp = random.randint(10, 15)
    else:
        new_xp = random.randint(0, 5)
    
    xp = int(current_xp + new_xp)
    ...
    database_interaction.update_user_entry(user_id=user_id, increment=False, msg_count=msg_count, xp=xp, level=level, cmds_used=cmds_used, reported=reported, been_reported=been_reported, nsfw_opt_out=nsfw_opt_out)
